...
-  h (helper) - класс используемый много раз, чтобы не дублироавть код (тоже самое, что бэм блок, но вложенных элементов нет).
...
-  src - папка с исходниками проекта.
   ...
   -  scss
      -  common - заготовка стилей для сайтов, которые будут меняться под конкретный сайт.
      ...
      -  utils - стили переходящие из проекта в проект (т.е. css helpers).
      -  site-main.scss - файл, который подключает все нужные файлы стилей и он же и компилируется gulp задачей.
      -  trash-styles.scss - файл стилей для нового верстальщика проекта, если лень искать нужный файл стилей.
   -  svg - svg иконки.
